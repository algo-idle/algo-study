...
## 문제 설명
```text
1. N, M과 메모리, 비용을 입력받는다.
2. 0-1배낭 문제와 동일하게 작성한다.
3. 출력할 때, dp 테이블의 가장 마지막 리스트 중, 가장 먼저 M 이상의 메모리를 확보한 인덱스 번호를 출력한다. 
```
...
m = [0] + list(map(int, input().rstrip().split()))
c = [0] + list(map(int, input().rstrip().split()))
